...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/presenter-account-api/env

    PORT="${PRESENTER_ACCOUNT_API_PORT}"
...
exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" "${APP_DIR}/presenter-account-api.jar"

# debug settings
#exec java -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=21097 -jar -Dserver.port="${PORT}" "${APP_DIR}/presenter-account-api.jar"
