...
public final class EmailValidator {
    ...
    private static final String EMAIL_FORMAT = "^["+ VALID_CHARACTERS +"]+@["+ VALID_CHARACTERS +"]+\\.["+ VALID_CHARACTERS +"]+$";
     ...
     */
    public static final Optional<String> validateEmail(final String unvalidatedEmail, final int maxLength) throws IllegalArgumentException {
        ...
        if(validLength && validToParse){
            ...
            if (Pattern.matches(EMAIL_FORMAT, unvalidEmailUTF8) && unvalidEmailUTF8.length() <= maxLength){
        ...
        if(!validLength){
            final String additionalInfo = String.format("Email was %d when it should be between %d and %d", maxLength, MIN_MAX_LENGTH, MAX_LENGTH);
